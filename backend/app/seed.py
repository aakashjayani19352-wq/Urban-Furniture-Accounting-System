...
def seed_database():
    ...
    try:
        ...
        print("Seeding Sample Purchase & Sales Orders...")
        from app.models import PurchaseOrder, SalesOrder
        sample_po = PurchaseOrder(
            vendor_id=vendor.id,
            product_id=p1.id,
            quantity=5,
            unit_price=p1.cost_price,
            total_amount=5 * p1.cost_price,
            status="draft"
        )
        sample_so = SalesOrder(
            customer_id=customer.id,
            product_id=p2.id,
            quantity=3,
            unit_price=p2.sales_price,
            tax=50.0,
            total_amount=(3 * p2.sales_price) + 50.0,
            status="draft"
        )
        db.add_all([sample_po, sample_so])
        db.commit()
